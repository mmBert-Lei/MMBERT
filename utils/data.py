...
class Data:
    ...
    def build_alphabet(self, input_file):
        ...
        for idx in range(len(in_lines)):
            ...
            if len(line) > 2:
                ...
                label = pairs[-1]
                entity = label[2:]
                if entity and entity not in self.type2id.keys() :
                    self.type_index += 1
                    self.type2id[entity] = self.type_index
                self.label_alphabet.add(label)
    ...
    def build_gaz_alphabet(self, input_file):
        ...
        #for idx,line in enumerate(in_lines):
            if len(line) > 3 :
            #if len(line) > 3 and (idx+1) % self.MAX_SENTENCE_LENGTH != 0:
    ...
    def read_instance_with_gaz(self,input_file, gaz, word_alphabet, biword_alphabet, char_alphabet, gaz_alphabet,
                               label_alphabet,
        ...
        for idx in range(len(in_lines)):
            ...
            if pairs[0] != '。':
                ...
                if label.startswith('B'):
                    ...
                    label_index.append(iddx)
                    entity_type = label[2:]
                    label_indexs.append({'index': label_index, 'type':entity_type })
                ...
                words.append(word)
                labels.append(label)
                word_Ids.append(token2id(word))
                biword_Ids.append(token2id(biword))
                label_Ids.append(token2id(label))
                char_list = []
                char_Id = []
                for char in word:
                ...
                if ((max_sent_length < 0) or (len(words) < max_sent_length)) and (len(words) > 0):
                    ...
                    gaz_Ids = []
                    w_length = len(words)
                    _grid_labels = np.zeros((w_length, w_length), dtype=np.int)
                    # print(words)
                    # print(label_indexs)
                    all_label_indexs.append(label_indexs)
                    for entity in label_indexs:
                        index = entity["index"]
                        for i in range(len(index)):
                            if i + 1 >= len(index):
                                break
                            # print('i',i)
                            # print('_grid_labels',_grid_labels.shape)
                            # print('index[i]',index[i + 1])
                            _grid_labels[index[i], index[i + 1]] = 1
                        _grid_labels[index[-1], index[0]] = self.type2id[entity["type"]]
                    grid_labels.append(_grid_labels)
                    all_lenth.append(w_length)
                    for idx in range(w_length):
                        ...
                        if matched_Id:
                            ...
                            gaz_Ids.append([])
                    # gazs: 能组成词的 如['新华','新华社'] chars :每个字 [['新'],['华'],['社']]
                    # biwords 和后一个字组成的二字词 ['新华','华社']
                    # words 每个字符 ['新','华','社']
                ...
                gazs = []
                gaz_Ids = []
        # tys = instence_texts[645]
        # tysid = instence_Ids[645]
        # tysilb = grid_labels[645]
        # word_Idsd = instence_Ids[645]
        # windex = all_label_indexs[645]
        # all_lenthx =all_lenth[645]
        return instence_Ids, grid_labels, instence_texts
    def generate_instance_with_gaz(self, input_file):
        self.fix_alphabet()
        return self.read_instance_with_gaz(input_file, self.gaz, self.word_alphabet, self.biword_alphabet, self.char_alphabet, self.gaz_alphabet,  self.label_alphabet, self.number_normalized, self.MAX_SENTENCE_LENGTH)
